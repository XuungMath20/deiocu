...
local redzlib = {
    Themes = {
        Cyberpunk = {
            ...
            ["Color Element Stroke"] = Color3.fromRGB(255, 255, 255)
        }
    }, 
    Info = {
        Version = "1.2.0"
...
function redzlib:MakeWindow(Configs)
    local WTitle = Configs[1] or Configs.Name or Configs.Title or "Bazuka Hub"
    local WMiniText = Configs[2] or Configs.SubTitle or "Slayer"
    ...
    local SizeControlButton = Create("ImageButton", {
        Size = UDim2.new(0, 20, 0, 20),
        Position = UDim2.new(1, -60, 0.5),
        AnchorPoint = Vector2.new(1, 0.5),
        BackgroundTransparency = 1,
        Image = "rbxassetid://6966627582",
        ImageColor3 = Theme["Color Text"],
        AutoButtonColor = false,
        Name = "SizeControl"
    })

    local SizeTooltip = Create("Frame", ScreenGui, {
    ...
    Make("Stroke", SizeTooltip, {Color = Theme["Color Stroke"]})
    Make("Corner", SizeTooltip, UDim.new(0, 6))
    
    local TooltipText = InsertTheme(Create("TextLabel", SizeTooltip, {
        Size = UDim2.new(1, -10, 1, -10),
        Position = UDim2.new(0.5, 0, 0.5, 0),
        AnchorPoint = Vector2.new(0.5, 0.5),
        BackgroundTransparency = 1,
        Text = "Click to change size",
        TextColor3 = Theme["Color Text"],
        TextSize = 10,
        Font = Enum.Font.Gotham,
        RichText = true
    ...
    local SizeStates = {"Small", "Medium", "Large", "Default"}
    local CurrentSizeState = 3
    
    function Window:CycleWindowSize()
        CurrentSizeState = CurrentSizeState + 1
        if CurrentSizeState > #SizeStates then
            CurrentSizeState = 1
        end
        
        local newSize
        if CurrentSizeState == 1 then
            newSize = UDim2.fromOffset(450, 300)
            MainScroll.Size = UDim2.new(0, 130, 1, -28)
            SizeControlButton.ImageColor3 = Color3.fromRGB(200, 200, 200)
        elseif CurrentSizeState == 2 then
            newSize = UDim2.fromOffset(550, 380)
            MainScroll.Size = UDim2.new(0, 140, 1, -28)
            SizeControlButton.ImageColor3 = Color3.fromRGB(220, 220, 220)
        elseif CurrentSizeState == 3 then
            newSize = UDim2.fromOffset(630, 440)
            MainScroll.Size = UDim2.new(0, 150, 1, -28)
            SizeControlButton.ImageColor3 = Theme["Color Text"]
        else
            newSize = UDim2.fromOffset(unpack(redzlib.Save.UISize))
            MainScroll.Size = UDim2.new(0, redzlib.Save.TabSize, 1, -28)
            SizeControlButton.ImageColor3 = Theme["Color Text"]
        end
        Containers.Size = UDim2.new(1, -MainScroll.Size.X.Offset, 1, -28)
        CreateTween({MainFrame, "Size", newSize, 0.3, true})
        redzlib.Save.UISize = {newSize.X.Offset, newSize.Y.Offset}
        redzlib.Save.TabSize = MainScroll.Size.X.Offset
        SaveJson("Ghost library V5.json", redzlib.Save)
        ShowSizeTooltip(SizeStates[CurrentSizeState])
    end

    function ShowSizeTooltip(state)
        TooltipText.Text = "Size: " .. state
        SizeTooltip.Position = UDim2.fromOffset(
            SizeControlButton.AbsolutePosition.X + SizeControlButton.AbsoluteSize.X/2 - 60,
            SizeControlButton.AbsolutePosition.Y - 35
        )
        SizeTooltip.Visible = true
        CreateTween({SizeTooltip, "BackgroundTransparency", 0.25, 0.1})
        
        task.wait(2)
        CreateTween({SizeTooltip, "BackgroundTransparency", 1, 0.3, true})
        SizeTooltip.Visible = false
    end

    SizeControlButton.MouseEnter:Connect(function()
        CreateTween({SizeControlButton, "ImageColor3", Theme["Color Theme"], 0.2})
        SizeTooltip.Position = UDim2.fromOffset(
            SizeControlButton.AbsolutePosition.X + SizeControlButton.AbsoluteSize.X/2 - 60,
            SizeControlButton.AbsolutePosition.Y - 35
        )
    ...
    SizeControlButton.MouseLeave:Connect(function()
        CreateTween({SizeControlButton, "ImageColor3", Theme["Color Text"], 0.2})
        CreateTween({SizeTooltip, "BackgroundTransparency", 1, 0.3, true})
        task.wait(0.3)
        SizeTooltip.Visible = false
    end)

    SizeControlButton.Activated:Connect(function()
        Window:CycleWindowSize()
    end)
    ...
    function Window:MakeTab(paste, Configs)
        ...
        local TName = Configs[1] or Configs.Title or "Slayer"
        ...
        function Tab:AddTextBox(Configs)
            ...
            local TDesc = Configs.Desc or Configs.Description or ""
            local TDefault = Configs[2] or Configs.Default or "1"
            local TPlaceholderText = Configs[5] or Configs.PlaceholderText or "Input"
            local TClearText = Configs[3] or Configs.ClearText or false
            local Callback = Funcs:GetCallback(Configs, 4)
            
            if TDefault == nil or TDefault == "" or (type(TDefault) == "string" and TDefault:gsub(" ", ""):len() < 1) then
                TDefault = "1"
            end
            ...
            local SelectedFrame = InsertTheme(Create("Frame", Button, {
                ...
                Position = UDim2.new(1, -10, 0.5),
                AnchorPoint = Vector2.new(1, 0.5),
                BackgroundColor3 = Theme["Color Element"] or Theme["Color Hub 2"],
                BackgroundTransparency = 0.25
            }), "Frame")
            
            Make("Stroke", SelectedFrame, {Color = Theme["Color Element Stroke"] or Theme["Color Stroke"]})
            ...
            function TextBox:Set(NewValue)
                TextBoxInput.Text = tostring(NewValue)
                Input()
            end
